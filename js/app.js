...
function renderProducts() {
  ['electronics', 'food', 'home'].forEach(cat => {
    ...
    if (!grid) return;
    grid.innerHTML = PRODUCTS[cat].map(buildCardHTML).join('');
  });
  renderIcons();
}
    ...
    if (compareBtn) {
      ...
      if (idx > -1) {
        ...
        if (compareList.length >= 4) {
          showToast('⚠️ You can compare up to 4 products at a time');
          return;
        }
        compareList.push(id);
        const product = findProduct(id);
    ...
    if (id) {
      ...
      html += `
        <div style="flex:1;background:#f9fafb;border-radius:12px;padding:12px;min-width:0">
          ...
          <button onclick="removeFromCompare(${id})" style="margin-top:8px;width:100%;border:none;background:#fee2e2;color:#ef4444;font-family:'Work Sans',sans-serif;font-size:11px;font-weight:600;padding:5px;border-radius:8px;cursor:pointer">Remove</button>
        </div>`;
    } else {
      html += `
...
function navigateToCompare() {
  if (!compareList.length) {
    showToast('Add products to compare first');
    return;
  }
...
function initCompareDrawer() {
  document.getElementById('open-compare-drawer').addEventListener('click', e => {
    e.preventDefault();
  ...
  document.getElementById('close-compare-drawer').addEventListener('click', closeCompareDrawer);
...
function initAuthToggle() {
  ...
  if (signInBtn) signInBtn.addEventListener('click', e => {
    e.preventDefault();
    if (window.authShow) window.authShow('login');
  });
...
function initMobileBottomNav() {
  ...
  tabs.forEach(tab => {
    ...
    tab.addEventListener('click', () => {
      ...
      if (tabName === 'account') showToast('Sign in to view your account');
    });
  });
}
          ...
          <div class="history-card bg-white rounded-[16px] border border-[#f3f4f6] overflow-hidden" style="box-shadow:0 1px 4px rgba(0,0,0,0.05)">
            ...
            <div class="p-4">
              ...
              <div class="flex gap-2">
                <button onclick="deleteCompareHistory(${idx})" class="flex-1 bg-[#fee2e2] text-[#ef4444] font-work font-semibold text-[12px] py-2 rounded-[10px] hover:bg-[#fecaca] transition-colors">Delete</button>
...
window.deleteCompareHistory = function(idx) {
  let user = getDashboardData();
  if (user.compareHistory && user.compareHistory[idx]) {
    ...
    showToast('Compare session deleted');
  }
};
