  ...
  <div class="w-full max-w-[1290px] mx-auto px-8 flex items-center justify-between">
    ...
    <div class="flex items-center gap-8">
      ...
      <div class="flex items-center gap-3">
        ...
        <button id="header-sign-in-btn" onclick="window.authShow('login')" class="border border-[#e5e7eb] text-[#364153] font-work font-medium text-[14px] px-4 py-1.5 rounded-[14px] hover:bg-[#f9fafb] transition-colors">Sign in</button>
        ...
        <div id="header-user-area" class="hidden relative">
          ...
          <div id="header-user-dropdown" class="hidden absolute right-0 top-[calc(100%+8px)] w-[200px] bg-white rounded-[14px] border border-[#f3f4f6] py-1.5 z-50" style="box-shadow:0 8px 24px rgba(0,0,0,0.12)">
            ...
            <button onclick="window.authSignOut()" class="flex items-center gap-2.5 px-4 py-2.5 font-work font-medium text-[13px] text-[#ef4444] hover:bg-[#fff5f5] transition-colors w-full text-left">
      ...
      <div class="flex items-center gap-3">
        ...
        <div id="header-vendor-area" class="relative">
          ...
          <div id="header-vendor-dropdown" class="hidden absolute right-0 top-[calc(100%+8px)] w-[220px] bg-white rounded-[14px] border border-[#f3f4f6] py-1.5 z-50" style="box-shadow:0 8px 24px rgba(0,0,0,0.12)">
            ...
            <hr class="border-[#f3f4f6] my-1">
            <button onclick="window.vendorSignOut()" class="flex items-center gap-2.5 px-4 py-2.5 font-work font-medium text-[13px] text-[#ef4444] hover:bg-[#fff5f5] transition-colors w-full text-left">
              <i data-lucide="log-out" class="w-3.5 h-3.5 shrink-0"></i> Sign Out
            </button>
          </div>
        </div>
...
<nav class="sm:hidden fixed bottom-0 left-0 right-0 bg-white z-50 flex items-center" style="height:71px;border-top:1px solid #f3f4f6;box-shadow:0 -4px 20px rgba(0,0,0,0.07)">
  ...
  const CLOSE_BTN = `<button onclick="window.authClose()" class="absolute top-4 right-4 w-8 h-8 rounded-full bg-[#f3f4f6] flex items-center justify-center hover:bg-[#e5e7eb] transition-colors" title="Close">
    <i data-lucide="x" class="w-4 h-4 text-[#6a7282]"></i>
  </button>`;

  const AUTH_MODALS_HTML = `
<!-- ── LOGIN MODAL ─────────────────────────────────────── -->
<div id="auth-login" class="hidden fixed inset-0 z-[201] flex items-center justify-center px-4 overflow-y-auto py-8" style="background:rgba(0,0,0,0.5)" onclick="window.authClose()">
  <div class="relative bg-white rounded-[20px] w-full max-w-[400px] p-8 flex flex-col gap-4 my-auto" style="box-shadow:0 24px 64px rgba(0,0,0,0.2)" onclick="event.stopPropagation()">
    ...
    <div class="flex flex-col gap-3">
      <div>
        ...
        <input id="login-identifier" type="text" placeholder="Enter email or phone number" class="auth-input w-full border border-[#e5e7eb] rounded-[12px] px-4 py-3 font-work text-[14px] text-[#101828] outline-none focus:border-[#155dfc] transition-colors">
      ...
      <div>
        ...
        <div class="relative">
          <input id="login-password" type="password" placeholder="••••••••••••" class="auth-input w-full border border-[#e5e7eb] rounded-[12px] px-4 py-3 font-work text-[14px] text-[#101828] outline-none focus:border-[#155dfc] transition-colors pr-11">
          <button type="button" onclick="authTogglePwd('login-password',this)" class="absolute right-3 top-1/2 -translate-y-1/2 text-[#99a1af] hover:text-[#6a7282]">
            <i data-lucide="eye" class="w-4 h-4"></i>
          </button>
        </div>
        <p id="login-password-err" class="auth-err hidden font-work text-[11px] text-[#ef4444] mt-1"></p>
        <div class="text-right mt-1.5">
          <button onclick="authShow('forgot')" class="font-work font-medium text-[12px] text-[#155dfc] hover:underline">Forgot Password?</button>
        </div>
      </div>
    </div>
    <button onclick="authDoSignIn()" class="w-full bg-[#155dfc] text-white font-work font-semibold text-[15px] py-3 rounded-[12px] hover:bg-[#1447e6] transition-colors">Sign In</button>
    <button onclick="authGoogleSignIn()" class="w-full flex items-center justify-center gap-2 border border-[#e5e7eb] text-[#364153] font-work font-semibold text-[14px] py-3 rounded-[12px] hover:bg-[#f9fafb] transition-colors">
      ${GOOGLE_SVG} Continue with Google
    </button>
    <p class="text-center font-work text-[13px] text-[#6a7282]">Don't have an account? <button onclick="authShow('signup')" class="text-[#155dfc] font-semibold hover:underline">Sign Up</button></p>
  </div>
</div>

<!-- ── CREATE ACCOUNT MODAL ────────────────────────────── -->
<div id="auth-signup" class="hidden fixed inset-0 z-[201] flex items-center justify-center px-4 overflow-y-auto py-8" style="background:rgba(0,0,0,0.5)" onclick="window.authClose()">
  <div class="relative bg-white rounded-[20px] w-full max-w-[400px] p-8 flex flex-col gap-4 my-auto" style="box-shadow:0 24px 64px rgba(0,0,0,0.2)" onclick="event.stopPropagation()">
    ${CLOSE_BTN}
    <div class="text-center">
    ...
    <div class="flex flex-col gap-3">
      ...
      <div>
        ...
        <div class="relative">
          ...
          <button type="button" onclick="authTogglePwd('signup-password',this)" class="absolute right-3 top-1/2 -translate-y-1/2 text-[#99a1af] hover:text-[#6a7282]">
            <i data-lucide="eye" class="w-4 h-4"></i>
          </button>
        </div>
    ...
    <button onclick="authDoSignUp()" class="w-full bg-[#155dfc] text-white font-work font-semibold text-[15px] py-3 rounded-[12px] hover:bg-[#1447e6] transition-colors">Continue</button>
    <button onclick="authGoogleSignIn()" class="w-full flex items-center justify-center gap-2 border border-[#e5e7eb] text-[#364153] font-work font-semibold text-[14px] py-3 rounded-[12px] hover:bg-[#f9fafb] transition-colors">
      ${GOOGLE_SVG} Continue with Google
    </button>
    <p class="text-center font-work text-[13px] text-[#6a7282]">Already have an account? <button onclick="authShow('login')" class="text-[#155dfc] font-semibold hover:underline">Sign In</button></p>
  </div>
</div>

<!-- ── OTP MODAL (Sign Up) ─────────────────────────────── -->
<div id="auth-otp" class="hidden fixed inset-0 z-[201] flex items-center justify-center px-4" style="background:rgba(0,0,0,0.5)" onclick="window.authClose()">
  <div class="relative bg-white rounded-[20px] w-full max-w-[380px] p-8 flex flex-col items-center gap-5" style="box-shadow:0 24px 64px rgba(0,0,0,0.2)" onclick="event.stopPropagation()">
    ...
    <div class="flex items-center gap-3">
      <input type="text" maxlength="1" oninput="otpNext(this,0,'otp-inputs')" onkeydown="otpBack(event,this)" class="otp-box w-12 h-14 text-center border border-[#e5e7eb] rounded-[12px] font-work font-bold text-[22px] text-[#101828] outline-none focus:border-[#155dfc] transition-colors">
      <input type="text" maxlength="1" oninput="otpNext(this,1,'otp-inputs')" onkeydown="otpBack(event,this)" class="otp-box w-12 h-14 text-center border border-[#e5e7eb] rounded-[12px] font-work font-bold text-[22px] text-[#101828] outline-none focus:border-[#155dfc] transition-colors">
      <input type="text" maxlength="1" oninput="otpNext(this,2,'otp-inputs')" onkeydown="otpBack(event,this)" class="otp-box w-12 h-14 text-center border border-[#e5e7eb] rounded-[12px] font-work font-bold text-[22px] text-[#101828] outline-none focus:border-[#155dfc] transition-colors">
      <input type="text" maxlength="1" oninput="otpNext(this,3,'otp-inputs')" onkeydown="otpBack(event,this)" class="otp-box w-12 h-14 text-center border border-[#e5e7eb] rounded-[12px] font-work font-bold text-[22px] text-[#101828] outline-none focus:border-[#155dfc] transition-colors">
      <input type="text" maxlength="1" oninput="otpNext(this,4,'otp-inputs')" onkeydown="otpBack(event,this)" class="otp-box w-12 h-14 text-center border border-[#e5e7eb] rounded-[12px] font-work font-bold text-[22px] text-[#101828] outline-none focus:border-[#155dfc] transition-colors">
    </div>
    <p id="otp-err" class="auth-err hidden font-work text-[11px] text-[#ef4444] -mt-2"></p>
    <button onclick="authDoOTP('otp-inputs','dashboard.html')" class="w-full bg-[#155dfc] text-white font-work font-semibold text-[15px] py-3 rounded-[12px] hover:bg-[#1447e6] transition-colors">Verify</button>
...
<div id="auth-forgot" class="hidden fixed inset-0 z-[201] flex items-center justify-center px-4" style="background:rgba(0,0,0,0.5)" onclick="window.authClose()">
  <div class="relative bg-white rounded-[20px] w-full max-w-[400px] p-8 flex flex-col gap-5" style="box-shadow:0 24px 64px rgba(0,0,0,0.2)" onclick="event.stopPropagation()">
    ${CLOSE_BTN}
    <button onclick="authShow('login')" class="flex items-center gap-2 text-[#6a7282] hover:text-[#364153] transition-colors self-start -mb-1">
    ...
    <button onclick="authDoForgot()" class="w-full bg-[#155dfc] text-white font-work font-semibold text-[15px] py-3 rounded-[12px] hover:bg-[#1447e6] transition-colors">Send Reset Code</button>
  </div>
</div>

<!-- ── FORGOT PASSWORD — OTP ───────────────────────────── -->
<div id="auth-forgot-otp" class="hidden fixed inset-0 z-[201] flex items-center justify-center px-4" style="background:rgba(0,0,0,0.5)" onclick="window.authClose()">
  <div class="relative bg-white rounded-[20px] w-full max-w-[380px] p-8 flex flex-col items-center gap-5" style="box-shadow:0 24px 64px rgba(0,0,0,0.2)" onclick="event.stopPropagation()">
    ${CLOSE_BTN}
    <div class="w-16 h-16 rounded-full bg-[#eff6ff] flex items-center justify-center">
    ...
    <div class="flex items-center gap-3">
      <input type="text" maxlength="1" oninput="otpNext(this,0,'forgot-otp-inputs')" onkeydown="otpBack(event,this)" class="fotp-box w-12 h-14 text-center border border-[#e5e7eb] rounded-[12px] font-work font-bold text-[22px] text-[#101828] outline-none focus:border-[#155dfc] transition-colors">
      <input type="text" maxlength="1" oninput="otpNext(this,1,'forgot-otp-inputs')" onkeydown="otpBack(event,this)" class="fotp-box w-12 h-14 text-center border border-[#e5e7eb] rounded-[12px] font-work font-bold text-[22px] text-[#101828] outline-none focus:border-[#155dfc] transition-colors">
      <input type="text" maxlength="1" oninput="otpNext(this,2,'forgot-otp-inputs')" onkeydown="otpBack(event,this)" class="fotp-box w-12 h-14 text-center border border-[#e5e7eb] rounded-[12px] font-work font-bold text-[22px] text-[#101828] outline-none focus:border-[#155dfc] transition-colors">
      <input type="text" maxlength="1" oninput="otpNext(this,3,'forgot-otp-inputs')" onkeydown="otpBack(event,this)" class="fotp-box w-12 h-14 text-center border border-[#e5e7eb] rounded-[12px] font-work font-bold text-[22px] text-[#101828] outline-none focus:border-[#155dfc] transition-colors">
      <input type="text" maxlength="1" oninput="otpNext(this,4,'forgot-otp-inputs')" onkeydown="otpBack(event,this)" class="fotp-box w-12 h-14 text-center border border-[#e5e7eb] rounded-[12px] font-work font-bold text-[22px] text-[#101828] outline-none focus:border-[#155dfc] transition-colors">
    </div>
    <p id="fotp-err" class="auth-err hidden font-work text-[11px] text-[#ef4444] -mt-2"></p>
    <button onclick="authDoOTP('forgot-otp-inputs','forgot-success')" class="w-full bg-[#155dfc] text-white font-work font-semibold text-[15px] py-3 rounded-[12px] hover:bg-[#1447e6] transition-colors">Verify Code</button>
...
<div id="auth-forgot-success" class="hidden fixed inset-0 z-[201] flex items-center justify-center px-4" style="background:rgba(0,0,0,0.5)" onclick="window.authClose()">
  <div class="relative bg-white rounded-[20px] w-full max-w-[380px] p-8 flex flex-col items-center gap-5 text-center" style="box-shadow:0 24px 64px rgba(0,0,0,0.2)" onclick="event.stopPropagation()">
    ...
    <button onclick="authShow('new-password')" class="w-full bg-[#155dfc] text-white font-work font-semibold text-[15px] py-3 rounded-[12px] hover:bg-[#1447e6] transition-colors">Set New Password</button>
  </div>
</div>

<!-- ── FORGOT PASSWORD — CHANGE PASSWORD ──────────────── -->
<div id="auth-new-password" class="hidden fixed inset-0 z-[201] flex items-center justify-center px-4" style="background:rgba(0,0,0,0.5)" onclick="window.authClose()">
  <div class="relative bg-white rounded-[20px] w-full max-w-[400px] p-8 flex flex-col gap-5" style="box-shadow:0 24px 64px rgba(0,0,0,0.2)" onclick="event.stopPropagation()">
    ${CLOSE_BTN}
    <div class="text-center">
      <div class="w-14 h-14 rounded-full bg-[#eff6ff] flex items-center justify-center mx-auto mb-3">
    ...
    <div class="flex flex-col gap-3">
      <div>
        ...
        <div class="relative">
          ...
          <button type="button" onclick="authTogglePwd('newpwd-new',this)" class="absolute right-3 top-1/2 -translate-y-1/2 text-[#99a1af] hover:text-[#6a7282]">
            <i data-lucide="eye" class="w-4 h-4"></i>
          </button>
        </div>
      ...
      <div>
        ...
        <div class="relative">
          ...
          <button type="button" onclick="authTogglePwd('newpwd-confirm',this)" class="absolute right-3 top-1/2 -translate-y-1/2 text-[#99a1af] hover:text-[#6a7282]">
            <i data-lucide="eye" class="w-4 h-4"></i>
          </button>
        </div>
    ...
    <button onclick="authDoNewPassword()" class="w-full bg-[#155dfc] text-white font-work font-semibold text-[15px] py-3 rounded-[12px] hover:bg-[#1447e6] transition-colors">Update Password</button>
  </div>
</div>`;
  ...
  window.authShow = function(modal) {
  ...
  window.authTogglePwd = function(id, btn) {
  ...
  window.authGoogleSignIn = function() {
  ...
  window.otpNext = function(input, idx) {
  ...
  window.otpBack = function(event, input) {
  ...
  window.authDoSignIn = function() {
    ...
    if (typeof window.showToast === 'function') {
      window.showToast('👋 Welcome back, ' + displayName.split(' ')[0] + '!');
    }
  };

  /* ── Sign Up ── */
  window.authDoSignUp = function() {
  ...
  window.authDoForgot = function() {
    ...
    window.authShow('forgot-otp');
  };

  /* ── OTP verify (signup OTP → dashboard, forgot-pwd OTP → success screen) ── */
  window.authDoOTP = function(groupId, nextTarget) {
    ...
    if (nextTarget && nextTarget.includes('.html')) {
      ...
      window.authShow(nextTarget);
    }
  };

  /* ── New Password ── */
  window.authDoNewPassword = function() {
    ...
    window.authClose();
    if (typeof window.showToast === 'function') {
      window.showToast('✅ Password updated! Please sign in.');
    }
    setTimeout(() => window.authShow('login'), 400);
  };
